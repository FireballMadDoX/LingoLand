...
type GameType = 'dino' | 'bee' | 'scanner' | 'memory' | 'puzzle' | 'drawing' | null;
...
const MiniGamesHub: React.FC<MiniGamesHubProps> = ({ onBack }) => {
    ...
    if (activeGame === 'scanner') {
        return (
            <Suspense fallback={
                <div className="min-h-screen flex items-center justify-center" style={{ background: 'linear-gradient(160deg, #042F2E 0%, #064E3B 50%, #022C22 100%)' }}>
                    <div className="text-emerald-200 font-heading font-bold">Loading scanner…</div>
                </div>
            }>
                <Scanner onBack={() => setActiveGame(null)} />
            </Suspense>
        );
    }

    return (
        <div className="min-h-screen px-6 pb-16 pt-8" style={{ background: 'linear-gradient(160deg, #1E1B4B 0%, #2D1B69 50%, #1E1B4B 100%)' }}>
                    ...
                    {games.map((game, index) => (
                        <motion.div key={game.id}
                            ...
                            <div className="p-6">
                                ...
                                <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-3xl mb-4 shadow-lg" style={{ background: game.gradient }}>
                                    {game.icon}
                                </div>
                                ...
                                {!game.locked && (
                                    <div className="mt-5 inline-flex items-center gap-2 px-4 py-2 rounded-xl font-heading font-bold text-sm text-white"
